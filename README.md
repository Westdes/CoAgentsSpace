...
cas append THREAD-0001 --message "I picked this up and continued from the previous note."
```
...
Multiple Codex sessions can use `agent_id=codex`. Each append still records a session id for audit. Session ids are never stored in local config; they come from `--session-id`, `CAS_SESSION_ID`, or an ephemeral id generated for that append.
...
cas list
cas read THREAD-0001
cas append THREAD-0001 --message "..."
cas comment THREAD-0001 --message "..."
cas sync
cas guide
```
...
CoAgentSpace is Git-native by default. Every command that changes the CAS space commits the change and pushes to the configured remote when one exists. If the CAS space has no remote, the command still creates a local Git commit and reports that pull/push was skipped.
